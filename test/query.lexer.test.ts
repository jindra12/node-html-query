...
const parseQueue = (input: string) =>
    parseLexer(input, parsedQueryLexer, false).queue;
...
const complexQueryStructures: Record<string, string[]> = {
    ...
    "li:first-child(2n + 1 of .class > #id)": [
        "li",
        ":",
        "first-child(",
        "2",
    ...
    Object.entries(testMap).forEach(([key, tests]) => {
        ...
        tests.forEach((test) => {
            it(`${test.inverse ? "Doesn't match" : "Matches"
                } ${lexerType} with value ${test.value}, with regex ${typeof lexerValue === "string" ? lexerValue : lexerValue.source
                }`, () => {
                    if (test.inverse) {
